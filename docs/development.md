...
`start-stackpilot.bat` is the repository convenience launcher for the default current-user installation. Its first run installs and starts `dist/stackpilot.exe`. Every later run performs an authenticated graceful stop, upgrades the registered installation from the current `dist/stackpilot.exe`, starts a fresh control-plane PID, and opens a new browser session unless `--no-open` is supplied. It does not terminate managed business services; their detached Supervisors remain responsible for takeover and reconciliation.
