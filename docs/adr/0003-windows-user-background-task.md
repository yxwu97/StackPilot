...
10. The repository-only `start-stackpilot.bat` convenience launcher is intentionally restart-and-update, while the public `service start` command remains idempotent. After the first installation, each launcher invocation runs the authenticated `service stop`, updates the registered immutable executable through `service upgrade` using the repository's current `dist/stackpilot.exe`, and then runs `service start`. It never falls back to process-name or PID-only termination, and managed-service Supervisors remain independent across this control-plane restart.
